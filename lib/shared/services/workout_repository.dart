    ...
    try {
      ...
      for (final exercise in originalWorkout.exercises) {
        final duplicatedExercise = exercise.copyWith(
          ...
          series: exercise.series.map((s) => s.copyWith(
            id: null,
            done: false,
            isCompleted: false,
            repsDone: 0,
...
class CachedWorkoutRepository extends CachedRepository<Workout> with RepositoryMixin<Workout> {
  ...
  Future<List<Workout>> getByCompletionStatus(bool isCompleted) => _baseRepository.getByCompletionStatus(isCompleted);
  Future<List<Workout>> getOrderedByWeek(String weekId) => _baseRepository.getOrderedByWeek(weekId);
  Future<Workout?> getWithExercises(String workoutId) => _baseRepository.getWithExercises(workoutId);
  Future<void> updateCompletionStatus(String workoutId, bool isCompleted) => _baseRepository.updateCompletionStatus(workoutId, isCompleted);
  ...
  Future<void> updateNotes(String workoutId, String notes) => _baseRepository.updateNotes(workoutId, notes);
  Future<void> updateEstimatedDuration(String workoutId, int durationMinutes) => _baseRepository.updateEstimatedDuration(workoutId, durationMinutes);
  Future<String> duplicateWorkout(String workoutId, int newOrder, {String? newWeekId}) => _baseRepository.duplicateWorkout(workoutId, newOrder, newWeekId: newWeekId);
  Future<void> reorderWorkouts(List<Workout> workouts) => _baseRepository.reorderWorkouts(workouts);
  Future<WorkoutStats> getWorkoutStats(String workoutId) => _baseRepository.getWorkoutStats(workoutId);
  Future<List<Workout>> getPerformedInDateRange(DateTime startDate, DateTime endDate) => _baseRepository.getPerformedInDateRange(startDate, endDate);
  Future<void> deleteWithExercises(String workoutId) => _baseRepository.deleteWithExercises(workoutId);
  Stream<List<Workout>> listenByWeekId(String weekId) => _baseRepository.listenByWeekId(weekId);
  Stream<Workout?> listenWithExercises(String workoutId) => _baseRepository.listenWithExercises(workoutId);
}