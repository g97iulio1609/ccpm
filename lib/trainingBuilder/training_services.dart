...
class FirestoreService {
  ...
  Future<void> removeWorkout(String workoutId) async {
    ...
    var exerciseSnapshot = await _db
        .collection('exercisesWorkout')
      ...
      for (var workout in week.workouts) {
        ...
        for (var exercise in workout.exercises) {
          String exerciseId = exercise.id?.trim().isEmpty ?? true
              ? db.collection('exercisesWorkout').doc().id
              : exercise.id!;
          exercise.id = exerciseId;
          DocumentReference exerciseRef =
              db.collection('exercisesWorkout').doc(exerciseId);
          batch.set(
              exerciseRef,
              {
                'name': exercise.name,
          ...
          for (int i = 0; i < exercise.series.length; i++) {
            ...
            batch.set(
                seriesRef,
                {
                  ...
                  'serieId': seriesId,
                  'order':
                      i + 1, // Usa l'indice della serie come valore per 'order'
                  'done': series.done,
  ...
  Future<TrainingProgram> fetchTrainingProgram(String programId) async {
    ...
    for (int i = 0; i < weeks.length; i++) {
      Week week = weeks[i];
      List<Workout> workouts = workoutsSnapshots[i]
          .docs
      ...
      for (int j = 0; j < workouts.length; j++) {
        ...
        for (int k = 0; k < exercises.length; k++) {
          ...
          List<Series> seriesList = seriesSnapshots[k]
              .docs
